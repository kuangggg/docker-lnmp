...
if [[ -z "${EXTENSIONS##*,phptars,*}" ]]; then

    mkdir phptars \
    && tar -xf tars-extension.tgz -C phptars --strip-components=1
    ( cd phptars && phpize && ./configure && make ${MC} && make install )
...
if [[ -z "${EXTENSIONS##*,grpc,*}" ]]; then
    ...
    mkdir grpc \
    && tar -xf grpc-1.30.0.tgz -C grpc --strip-components=1
...
if [[ -z "${EXTENSIONS##*,protobuf,*}" ]]; then
    ...
    mkdir protobuf \
    && tar -xf protobuf-3.12.3.tgz -C protobuf --strip-components=1
    ( cd protobuf && phpize && ./configure && make ${MC} && make install )
